...
class OptunaHandler():
    ...
    def objective(self, trial, randomState):
        ...
        if self.model_name == "GRANDE":
            ...
            Model = self.model(self.cat_col, self.cat_idx)

            # Assign grande with hyperparameters
            Model.assign_new_parameters(param_grid)
        ...
        for index, value in enumerate(self.random_states):
            ...
            if self.model_name == "GRANDE":
                ...
                for j in tqdm(range(2)):
                    ...
                    Model.assign_new_parameters(best_params)
...
class ResultHandler():
    ...
    def explain(self):
        ...
        if self.model_name == "GRANDE":
            ...
            # Assign grande with best hyperparameters
            Model.assign_new_parameters(self.optimized_Hp)
        ...
        # XGBoost / CatBoost / LightGBM on the otherhand uses tree shap
            ...
            shap.partial_dependence_plot(feature_name, Model, X_test)
    ...
    def bar_plot(self):
        shap.initjs()
        shap.bar_plot(self.shap_values)

    def decision_plot(self):
        shap.initjs()
        shap.decision_plot(self.shap_explainer.expected_value, self.shap_values, self.shap_testData)

    def partial_dependence_plot(self, feature_name= "age"):
        shap.initjs()
        shap.partial_dependence_plot(feature_name, self.shap_model, self.shap_testData)

    def force_plot(self, instance_index = 0):
        shap.initjs()
        shap.force_plot(self.shap_explainer.expected_value, self.shap_values[instance_index], self.shap_testData.iloc[instance_index,:])
...
class ConfigSelection():
    ...
    def load_model_config(self, model_name):
        try:
            ...
            model_config = getattr(model_config_module, f'my{model_name}')
...
class ModelSelection():
    def __init__(self, model_name):
        ...
        model = getattr(model_files, f'my{model_name}', None)
